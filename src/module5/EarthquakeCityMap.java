...
public class EarthquakeCityMap extends PApplet {
    ...
    private void hideMarkers() {
        for (Marker marker : quakeMarkers) {
            ...
            if (lastClicked instanceof CityMarker && marker.getLocation().getDistance(lastClicked.getLocation()) <= ((EarthquakeMarker)marker).threatCircle())
                marker.setHidden(false);
        }
        for (Marker marker : cityMarkers) {
            if (marker != lastClicked)
                marker.setHidden(true);
            if (lastClicked instanceof EarthquakeMarker && marker.getLocation().getDistance(lastClicked.getLocation()) <= ((EarthquakeMarker)lastClicked).threatCircle())
                marker.setHidden(false);
        }
    }
